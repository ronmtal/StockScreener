...
# ==============================================================================
# הגדרת מחלקות האינדיקטורים (במקום קובץ נפרד)
# ==============================================================================

class SimpleMovingAverage:
    def __init__(self, length=20, source='Close'):
        self.length = length
        self.source = source
        self.sma_col_name = f'SMA_{self.length}'

    def calculate(self, df: pd.DataFrame) -> pd.DataFrame:
        df[self.sma_col_name] = df[self.source].rolling(window=self.length).mean()
        return df

    def get_signals(self, df: pd.DataFrame) -> dict:
        signals = {}
        if not df.empty and self.sma_col_name in df.columns and len(df) > 1:
            for i in range(1, min(3, len(df))):
                prev_row = df.iloc[-i]
                if prev_row['Low'] <= prev_row[self.sma_col_name] <= prev_row['High']:
...
class RSI:
    def __init__(self, length=14, source='Close'):
        self.length = length
        self.source = source
...
class MACD:
    def __init__(self, fast_length=12, slow_length=26, signal_length=9, source='Close'):
    ...
    def calculate(self, df: pd.DataFrame) -> pd.DataFrame:
        fast_ema = df[self.source].ewm(span=self.fast_length, adjust=False).mean()
        slow_ema = df[self.source].ewm(span=self.slow_length, adjust=False).mean()
        df[self.macd_line_col] = fast_ema - slow_ema
        df[self.signal_line_col] = df[self.macd_line_col].ewm(span=self.signal_length, adjust=False).mean()
        df[self.histogram_col] = df[self.macd_line_col] - df[self.signal_line_col]
        return df

    def get_signals(self, df: pd.DataFrame) -> dict:
        signals = {}
        if not df.empty and self.histogram_col in df.columns and len(df) > 1:
...
# ==============================================================================
# --- הגדרות ותצורות גלובליות ---
# ==============================================================================
FILTER_GROUPS_FILE = "filter_groups.json"

# Simplified AVWAP configs (removed dependency on external library)
AVWAP_CONFIGS = [
    {"name": "aVWAP #1", "show": True, "source": "hlc3", "color": "#f6c309", "anchor_date": "2021-07-21"},
    {"name": "aVWAP #2", "show": True, "source": "hlc3", "color": "#fb9800", "anchor_date": "2021-08-05"},
    {"name": "aVWAP #3", "show": True, "source": "hlc3", "color": "#fb6500", "anchor_date": "2021-09-07"},
    {"name": "aVWAP #4", "show": True, "source": "hlc3", "color": "#f60c0c", "anchor_date": "2021-05-19"}
]
...
def load_index_symbols(index_type: str) -> pd.DataFrame:
    ...
    try:
        if index_type == "S&P 500":
            ...
            symbols = ['AAPL', 'MSFT', 'GOOGL', 'AMZN', 'NVDA', 'TSLA', 'META', 'AVGO', 'COST', 'NFLX',
                      'ADBE', 'PEP', 'TMUS', 'CSCO', 'CMCSA', 'TXN', 'QCOM', 'HON', 'AMGN', 'SBUX']
            names = ['Apple Inc.', 'Microsoft Corporation', 'Alphabet Inc.', 'Amazon.com Inc.', 'NVIDIA Corporation',
                    'Tesla Inc.', 'Meta Platforms Inc.', 'Broadcom Inc.', 'Costco Wholesale Corporation', 'Netflix Inc.',
                    'Adobe Inc.', 'PepsiCo Inc.', 'T-Mobile US Inc.', 'Cisco Systems Inc.', 'Comcast Corporation',
                    'Texas Instruments Incorporated', 'QUALCOMM Incorporated', 'Honeywell International Inc.',
            ...
            symbols = ['AAPL', 'MSFT', 'UNH', 'GS', 'HD', 'CAT', 'MCD', 'AMGN', 'V', 'BA',
                      'TRV', 'AXP', 'JPM', 'IBM', 'JNJ', 'WMT', 'PG', 'CVX', 'MRK', 'DIS']
            names = ['Apple Inc.', 'Microsoft Corporation', 'UnitedHealth Group Incorporated', 'The Goldman Sachs Group Inc.',
                    'The Home Depot Inc.', 'Caterpillar Inc.', "McDonald's Corporation", 'Amgen Inc.', 'Visa Inc.',
                    'The Boeing Company', 'The Travelers Companies Inc.', 'American Express Company', 'JPMorgan Chase & Co.',
                    'International Business Machines Corporation', 'Johnson & Johnson', 'Walmart Inc.', 'The Procter & Gamble Company',
                    'Chevron Corporation', 'Merck & Co. Inc.', 'The Walt Disney Company']
            df = pd.DataFrame({'Symbol': symbols, 'Name': names})
        else:  # Russell 2000 - use a small sample
            symbols = ['IWM', 'VTWO', 'URTY', 'UWM', 'TWM']
            names = ['iShares Russell 2000 ETF', 'Vanguard Russell 2000 ETF', 'ProShares UltraPro Russell2000',
                    'ProShares Ultra Russell2000', 'ProShares UltraShort Russell2000']
            df = pd.DataFrame({'Symbol': symbols, 'Name': names})
        ...
        fallback_symbols = ['AAPL', 'MSFT', 'GOOGL', 'AMZN', 'TSLA', 'META', 'NVDA', 'NFLX', 'ADBE', 'CRM']
        fallback_names = ['Apple', 'Microsoft', 'Alphabet', 'Amazon', 'Tesla', 'Meta', 'NVIDIA', 'Netflix', 'Adobe', 'Salesforce']
        return pd.DataFrame({'Symbol': fallback_symbols, 'Name': fallback_names})


# --- פונקציות זיהוי תבניות נרות ---
def is_doji(row, threshold):
...
def is_hammer_or_hanging_man(row):
...
def fetch_stock_data(symbol: str, sma_configs: list, rsi_config: dict, macd_config: dict) -> dict:
    ...
    try:
        ...
        try:
            info = ticker.info
            if not info:
                return {"symbol": symbol, "error": f"לא נמצא מידע בסיסי עבור {symbol}."}
        except Exception:
        ...
        # Get historical data
        try:
            df_hist = ticker.history(period="2y")  # Reduced period for faster loading
            if df_hist.empty:
                return {"symbol": symbol, "error": f"לא נמצאו נתונים היסטוריים עבור {symbol}."}
        except Exception as e:
            return {"symbol": symbol, "error": f"שגיאה בקבלת נתונים היסטוריים: {e}"}
        ...
        for sma_config in sma_configs:
            if sma_config['enabled']:
                try:
                    sma_indicator = SimpleMovingAverage(length=sma_config['length'])
                    df_with_sma = sma_indicator.calculate(df_hist.copy())
                    if not df_with_sma.empty and sma_indicator.sma_col_name in df_with_sma.columns:
                        last_sma = df_with_sma[sma_indicator.sma_col_name].iloc[-1]
                        if pd.notna(last_sma):
                            sma_values[f"SMA_{sma_config['length']}"] = last_sma
                            signals = sma_indicator.get_signals(df_with_sma)
                            sma_signals[f"sma_touch_{sma_config['length']}"] = signals.get('sma_touch', False)
                except Exception:
                    continue
        ...
        if rsi_config['enabled']:
            try:
                ...
                if not df_with_rsi.empty and rsi_indicator.rsi_col_name in df_with_rsi.columns:
                    last_rsi = df_with_rsi[rsi_indicator.rsi_col_name].iloc[-1]
        ...
        if macd_config['enabled']:
            try:
                macd_indicator = MACD(fast_length=macd_config['fast'], slow_length=macd_config['slow'],
                ...
                if not df_with_macd.empty:
                    ...
                    macd_values = {k: v for k, v in macd_raw_signals.items() if k.startswith('last_')}
            except Exception:
                pass
        ...
        if not df_hist.empty:
            try:
                ...
                if is_hammer_or_hanging_man(last_candle):
                    if len(df_hist) >= 4 and all(df_hist['Close'].iloc[-4:-1].diff().dropna() > 0):
        ...
        try:
            annual_financials = get_financial_statement_data(ticker.income_stmt, 'annual')
            quarterly_financials = get_financial_statement_data(ticker.quarterly_income_stmt, 'quarterly')
            revenues_list = [d['revenue'] for d in annual_financials if pd.notna(d['revenue'])]
            net_income_list = [d['net_income'] for d in annual_financials if pd.notna(d['net_income'])]
            is_both_growing = is_consistently_increasing(revenues_list, 3) and is_consistently_increasing(net_income_list, 3)
        except Exception:
            annual_financials = []
        ...
        # Get current price
        try:
            price = info.get("currentPrice") or info.get("regularMarketPrice") or df_hist['Close'].iloc[-1]
        except Exception:
            price = None
...
def get_financial_statement_data(statement_df: pd.DataFrame, period: str) -> list:
        ...
        for date_idx, row in statement_df.tail(periods_to_take).iterrows():
            try:
                ...
                financial_data.append({
                    "date": f"{date_idx.year}-Q{date_idx.quarter}" if period == 'quarterly' else date_idx.strftime('%Y'),
                    "revenue": revenue,
...
def is_consistently_increasing(data: list, years: int = 3) -> bool:
    ...
    try:
        ...
        return all(sub_list[i] >= sub_list[i - 1] for i in range(1, len(sub_list)))
    except Exception:
        return False


def is_consistently_decreasing(data: list, years: int = 3) -> bool:
...
def run_data_fetch_and_process(symbols_df: pd.DataFrame) -> pd.DataFrame:
    ...
    symbols_to_process = symbols_df['Symbol'].tolist()[:20]  # Limit to first 20 for demo
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        future_to_symbol = {
            executor.submit(fetch_stock_data, s, st.session_state.sma_configs, st.session_state.rsi_config,
                            st.session_state.macd_config): s
            for s in symbols_to_process
        }
        
        for i, future in enumerate(as_completed(future_to_symbol), 1):
            symbol = future_to_symbol[future]
            status_placeholder.text(f"🔍 סורק {i}/{total_symbols}: {symbol_name_map.get(symbol, symbol)}")
    ...
    for prefix in ['sma', 'rsi', 'macd']:
        ...
        if values_col in df.columns:
            try:
                ...
                df = pd.concat([df.drop(columns=[values_col]), values_df], axis=1)
            except Exception:
                pass
        ...
        if signals_col in df.columns:
            try:
                ...
                df = pd.concat([df.drop(columns=[signals_col]), signals_df], axis=1)
            except Exception:
                pass
    ...
    final_data = {
        ...
        "Market Cap (B)": df["market_cap"].apply(lambda x: x / 1e9 if pd.notna(x) and x else None),
        "Volume": df["volume"],
    ...
    numeric_cols = ['Price', 'P/E', 'Forward P/E', 'Market Cap (B)', 'Volume', 'Average Volume']
...
def get_chart_data(symbol):
    ...
    try:
        ...
        for length in st.session_state.chart_sma_lengths:
            df_hist[f'SMA_{length}'] = df_hist['Close'].rolling(window=length).mean()

        # Add RSI if enabled
        if st.session_state.rsi_config['enabled']:
            try:
                df_hist = RSI(length=st.session_state.rsi_config['length']).calculate(df_hist)
            except Exception:
                pass
        ...
        if st.session_state.macd_config['enabled']:
            try:
                ...
                df_hist = MACD(fast_length=cfg['fast'], slow_length=cfg['slow'], signal_length=cfg['signal']).calculate(df_hist)
            except Exception:
                pass

        # Add candlestick patterns
        try:
            df_hist['is_doji'] = df_hist.apply(lambda row: is_doji(row, st.session_state.doji_threshold), axis=1)
            df_hist['is_hammer_or_hanging_man'] = df_hist.apply(is_hammer_or_hanging_man, axis=1)
        except Exception:
            pass
...
def save_filter_groups():
    """Save filter groups to file"""
    try:
        with open(FILTER_GROUPS_FILE, 'w', encoding='utf-8') as f:
            json.dump(st.session_state.filter_groups, f, indent=4, ensure_ascii=False)
    except IOError as e:
        st.error(f"שגיאה בשמירת קובץ הסינונים: {e}")


def apply_filters_to_df(df: pd.DataFrame, filter_conditions: list) -> pd.DataFrame:
    ...
    for condition in filter_conditions:
        ...
        try:
            ...
            if "עקביות" in col_name:
                data_col = "revenues_list_for_filter" if "הכנסות" in col_name else "net_income_list_for_filter"
                func = is_consistently_decreasing if "(יורדת)" in col_name else is_consistently_increasing
                filtered_df = filtered_df[
                    filtered_df[data_col].apply(lambda x: func(x, condition.get("years_value", 3)))]
                ...
                if sma_length:
                    ...
                    if touch_col in filtered_df.columns:
                        filtered_df = filtered_df[filtered_df[touch_col] == True]
            elif col_name == "חציית MACD שורית":
                if "Bullish MACD Cross" in filtered_df.columns:
                    filtered_df = filtered_df[filtered_df["Bullish MACD Cross"] == True]
            elif col_name == "חציית MACD דובית":
                if "Bearish MACD Cross" in filtered_df.columns:
                    filtered_df = filtered_df[filtered_df["Bearish MACD Cross"] == True]
            elif col_name == "הכנסות ורווחים עקביים":
                filtered_df = filtered_df[filtered_df["Is Both Consistently Growing"]]
                ...
                if pattern == "Doji":
                    filtered_df = filtered_df[filtered_df["Is Doji Last"] == True]
                elif pattern == "Hammer":
                    filtered_df = filtered_df[filtered_df["Is Hammer Last"] == True]
                elif pattern == "Hanging Man":
                    filtered_df = filtered_df[filtered_df["Is Hanging Man Last"] == True]
            else:
                # Numeric filters
                col_map = {
                    "מחיר ($)": "Price", 
                ...
                for sma_config in st.session_state.sma_configs:
                    if sma_config['enabled']:
                        col_map[f"SMA {sma_config['length']}"] = f"SMA_{sma_config['length']}"
...
def initialize_session_state():
    ...
    if "current_filter_group" not in st.session_state or st.session_state.current_filter_group not in st.session_state.filter_groups:
        st.session_state.current_filter_group = list(st.session_state.filter_groups.keys())[0]
...
def setup_display_settings():
    ...
    available_sma_lengths = [cfg['length'] for cfg in st.session_state.sma_configs if cfg['enabled']]
    st.session_state.chart_sma_lengths = st.multiselect(
        "הצג ממוצעים נעים בגרף:",
        options=available_sma_lengths,
        default=[length for length in st.session_state.chart_sma_lengths if length in available_sma_lengths],
        key="chart_sma_multiselect"
    )


def setup_filter_group_management():
    ...
    current_index = group_names.index(
        st.session_state.current_filter_group) if st.session_state.current_filter_group in group_names else 0
    ...
    if col2.button("✏️", help="שנה שם קבוצה", disabled=len(group_names) == 0): 
        st.session_state.rename_group_mode = True
    if col3.button("🗑️", help="מחק קבוצה", disabled=len(group_names) <= 1):
        del st.session_state.filter_groups[st.session_state.current_filter_group]
        st.session_state.current_filter_group = list(st.session_state.filter_groups.keys())[0]
        save_filter_groups()
        st.rerun()
    ...
    if st.session_state.rename_group_mode:
        with st.form("rename_group_form"):
            new_name = st.text_input("שם חדש לקבוצה:", value=st.session_state.current_filter_group)
            submitted = st.form_submit_button("שמור שינויים")
            if submitted and new_name and new_name != st.session_state.current_filter_group:
                if new_name not in st.session_state.filter_groups:
                    st.session_state.filter_groups[new_name] = st.session_state.filter_groups.pop(
...
def setup_filter_conditions():
    ...
    def draw_condition(i, condition):
        ...
        if col_type == "numeric":
            ...
            condition["operator"] = c2_1.selectbox(
                ...
                [">", "<", "=", ">=", "<="], 
                key=f"op_{i}",
                index=[">", "<", "=", ">=", "<="].index(condition.get("operator", ">"))
            )
            ...
            sma_options = [cfg['length'] for cfg in st.session_state.sma_configs if cfg['enabled']]
            if sma_options:
            ...
            condition["pattern_value"] = col2.selectbox(
                ...
                pattern_options, 
                key=f"pattern_{i}",
                index=pattern_options.index(condition.get("pattern_value", pattern_options[0]))
            )
...
def display_main_content(index_df: pd.DataFrame):
    ...
    if c1.button("🚀 סרוק נתונים", type="primary", use_container_width=True):
        with st.spinner("סורק נתונים, נא להמתין..."):
            st.session_state.full_df = run_data_fetch_and_process(index_df)
            st.session_state.filtered_df = st.session_state.full_df.copy() if st.session_state.full_df is not None else None
            st.session_state.previous_filtered_df = None
            st.success("סריקה הושלמה!")
            st.rerun()

    data_exists = st.session_state.full_df is not None and not st.session_state.full_df.empty
    
    if c2.button("🔍 החל סינון", use_container_width=True, disabled=not data_exists):
        st.session_state.previous_filtered_df = st.session_state.filtered_df.copy()
        conditions = st.session_state.filter_groups.get(st.session_state.current_filter_group, [])
        st.session_state.filtered_df = apply_filters_to_df(st.session_state.full_df.copy(), conditions)
    ...
    if c3.button("↩️ בטל סינון", use_container_width=True, disabled=not has_previous):
        st.session_state.filtered_df = st.session_state.previous_filtered_df.copy()
        st.session_state.previous_filtered_df = None
        st.rerun()

    if c4.button("🧹 אפס הכל", use_container_width=True, disabled=not data_exists):
...
def display_grid_view():
    ...
    if full_df is not None and not full_df.empty:
        ...
        if filtered_stocks == total_stocks:
            ...
            active_group = st.session_state.current_filter_group
            conditions_count = len(st.session_state.filter_groups.get(active_group, []))
    ...
    hidden_cols = [col for col in display_df.columns if
                   'data' in col.lower() or 'series' in col.lower() or 'filter' in col.lower() or 
                   'touch' in col.lower() or 'Cross' in col or 'Last' in col]
...
def display_single_stock_details(stock_series: pd.Series):
...
def create_combined_charts(symbol: str, stock_series: pd.Series) -> go.Figure | None:
    """Create combined price and indicator charts"""
    df_orig = get_chart_data(symbol)
    if df_orig is None or df_orig.empty:
        return go.Figure().update_layout(title=f"לא נמצאו נתונים עבור {symbol}")
    ...
    has_macd = (st.session_state.macd_config['enabled'] and 'MACD_line' in df_orig.columns)
    ...
    # Add price chart
    trace_params = {'x': df_display.index, 'name': 'מחיר', 'showlegend': False}
    ...
    elif st.session_state.chart_type == "Heikin Ashi" and not df_display.empty:
        trace_params.update({
    ...
    # Add MACD subplot
    if has_macd:
        colors_hist = ['green' if val >= 0 else 'red' for val in df_orig['MACD_histogram']]
...
def create_financials_chart(stock_series: pd.Series) -> go.Figure:
    ...
    data_key = "quarterly_financial_data" if period == "רבעוני" else "annual_financial_data"
    ...
    try:
        ...
        fig.add_trace(go.Bar(
            ...
            y=df['net_income'], 
            name='רווח נקי',
            marker_color=['green' if ni >= 0 else 'red' for ni in df['net_income']]
        ))
        ...
        return fig.update_layout(title=f"שגיאה בהצגת נתונים פיננסיים ({period})")
